                    ...
                    <div class="px-6 py-6">
                        ...
                        <div class="flex justify-end py-2 gap-2">
                            <button id="defaultModalButton" data-modal-target="createUserModal"
                                ...
                                focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 shadow-md hover:shadow-lg transition">
                                <!-- Plus Icon -->
                            ...
                            <button id="archiveModalButton" data-modal-target="archiveUserModal"
                                ...
                                focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 shadow-md hover:shadow-lg transition">
                                Archived Users
                            </button>
                        </div>
                    </div>
                                ...
                                <tr class="odd:bg-white even:bg-gray-50 border-b">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex gap-2">
                                            ...
                                            {{-- Hidden Archive Form --}}
                                            <form id="archive-form-{{ $user->id }}" action="{{ route('users.archive', $user->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('PUT')
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl p-6">
                <div class="flex items-center justify-between border-b pb-3 mb-4">
                    ...
                    <button type="button" data-modal-toggle="createUserModal"
                        class="p-2 rounded-full hover:bg-gray-200 text-gray-500 hover:text-gray-800">✕</button>
                </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr class="odd:bg-white even:bg-gray-50 border-b">
                                ...
                                <td class="px-6 py-4">{{ $user->email }}</td>
                                <td class="px-6 py-4">{{ $user->archived_at ? \Carbon\Carbon::parse($user->archived_at)->format('Y-m-d H:i') : '---' }}</td>
                                <td class="px-6 py-4">{{ $user->role ? ucfirst($user->role->name) : 'No Role' }}</td>
                                <td class="px-6 py-4">
                                    <form action="{{ route('users.unarchive', $user->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" class="px-3 py-2 text-xs text-white bg-green-600 hover:bg-green-700">
                                            Unarchive
                                        </button>
                                    </form>
                                </td>
