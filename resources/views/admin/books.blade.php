        ...
        <div class="lg:w-2/12 w-full">
            <div class="sticky top-28 space-y-6">
                ...
                <div class="bg-white p-5 rounded-[2rem] shadow-sm border border-gray-100 hidden lg:block">
                    <p class="text-[10px] font-black uppercase text-gray-400 px-2 mb-4 tracking-widest">Jump To Section
                    </p>
                    <nav class="flex flex-col gap-2">
                        <a href="#inventory"
                            class="px-4 py-2 text-xs font-bold text-gray-600 hover:bg-blue-600 hover:text-white rounded-xl transition-all">📚
                            Inventory</a>
                        <a href="#copies"
                            class="px-4 py-2 text-xs font-bold text-gray-600 hover:bg-blue-600 hover:text-white rounded-xl transition-all">🆔
                            Book Copies</a>
                        <a href="#authors"
                            class="px-4 py-2 text-xs font-bold text-gray-600 hover:bg-blue-600 hover:text-white rounded-xl transition-all">✍️
                            Authors</a>
                        <a href="#suppliers"
                            class="px-4 py-2 text-xs font-bold text-gray-600 hover:bg-blue-600 hover:text-white rounded-xl transition-all">🚚
                            Suppliers</a>
        ...
        <div class="lg:w-10/12 w-full space-y-12">
            ...
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <div class="relative w-full md:w-48 group">
                    <div class="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none">
                        <span
                            class="material-icons-outlined text-gray-400 group-focus-within:text-blue-600 transition-colors">event</span>
                    </div>
            ...
            {{-- SECTION: INVENTORY --}}
            <div id="inventory"
                class="section-container bg-white rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-8 border-b border-gray-50 flex justify-between items-center bg-white sticky top-0 z-10">
                    <div class="flex flex-col">
                        ...
                        <div class="flex items-center gap-2">
                            <p class="text-[10px] font-black uppercase text-blue-400">Total Entries: {{ $books->count()
                                }}</p>
                            ...
                            <div id="activeLocationContainer" class="hidden">
                                ...
                                <button onclick="clearLocationFilter()"
                                    class="bg-blue-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full flex items-center gap-1 hover:bg-red-500 transition-colors">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[600px] overflow-y-auto" id="bookGrid">
                    ...
                    <div class="search-item group bg-gray-50/50 border border-gray-100 p-6 rounded-[2.5rem] hover:bg-white hover:shadow-xl transition-all"
                        data-year="{{ $book->year_published }}" data-locations="{{ $locations->implode(',') }}">
                        <div class="flex flex-col h-full">
                            {{-- TOP ROW --}}
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3">
                                        <h3 class="text-lg font-black text-gray-800 search-text leading-tight">{{
                                            $book->title }}</h3>
                                        <button onclick='openBookModal(@json($book))'
                                ...
                                <div class="bg-gray-900 text-white px-4 py-2 rounded-2xl text-center min-w-[60px]">
                                    <span class="text-lg font-black block leading-none">{{ $book->copies->count()
                                        }}</span>
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center gap-2 text-gray-600">
                                    ...
                                    <span class="text-xs font-bold search-text">{{ $book->author?->name ?? 'Unknown
                                        Author' }}</span>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="flex items-center gap-2 text-gray-500">
                                        ...
                                        <span class="text-[11px] font-bold">Year: {{ $book->year_published ?? 'N/A'
                                            }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 gap-2">
                                <div class="flex items-center justify-between text-[10px]">
                                    ...
                                    <span class="font-bold text-gray-700 uppercase">{{ $book->publisher?->name ?? 'N/A'
                                        }}</span>
                                </div>
                                <div class="flex items-center justify-between text-[10px]">
                                    <span class="font-black uppercase text-gray-400">Supplier</span>
                                    <span class="font-bold text-blue-600 uppercase">{{ $book->supplier?->name ?? 'N/A'
                                        }}</span>
                                </div>
                            </div>
                        </div>
            ...
            {{-- SECTION: COPIES --}}
            <div id="copies"
                class="section-container bg-gray-900 rounded-[3rem] shadow-sm p-8 text-white overflow-hidden">
                <div class="flex justify-between items-center mb-8 px-4">
                    <h2 class="text-2xl font-black tracking-tighter">Physical Copies</h2>
                    <a href="{{ route('book-copies.index') }}"
                        class="text-[10px] font-black uppercase text-blue-400">Manage Detailed List &rarr;</a>
                </div>
                <div class="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div class="p-5 bg-white/5 border border-white/10 rounded-3xl flex items-center justify-between search-item"
                        ...
                        <div class="flex items-center gap-4">
                            <span
                                class="w-10 h-10 flex items-center justify-center bg-white/10 rounded-xl font-black text-xs text-blue-400">#{{
                                $copy->copy_number }}</span>
                            <span class="font-bold search-text">{{ $book->title }}</span>
                        </div>
                        <span
                            class="px-4 py-1.5 rounded-xl text-[9px] font-black uppercase {{ $copy->status == 'available' ? 'bg-green-500' : 'bg-orange-500' }} text-white">
                            {{ $copy->status }}
                        </span>
                    </div>
                    @endforeach
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
                <div id="authors"
                    class="section-container bg-white rounded-[3rem] shadow-sm border border-gray-100 p-8">
                    <div class="flex justify-between items-center mb-8">
                        <h2 class="text-2xl font-black text-gray-900 tracking-tighter leading-none">Authors</h2>
                        <a href="{{ route('authors.index') }}"
                            class="text-[10px] font-black uppercase text-blue-600">Manage List &rarr;</a>
                    </div>
                    <div class="grid grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2">
                        ...
                        <div class="p-4 bg-gray-50 rounded-2xl flex items-center gap-4 search-item group">
                            <div
                                class="w-10 h-10 bg-white rounded-xl flex items-center justify-center font-black text-xs text-gray-300 group-hover:bg-blue-600 group-hover:text-white transition-all">
                ...
                <div id="suppliers"
                    ...
                    <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                        ...
                        <div
                            class="p-6 bg-gray-50 rounded-[2rem] flex justify-between items-center search-item hover:bg-gray-100 transition-all group">
                            <div>
                                <h4 class="font-black text-gray-800 search-text">{{ $supplier->name }}</h4>
                                <p
                                    class="text-[10px] text-gray-400 font-bold mt-1 search-text uppercase tracking-widest">
                                    {{ $supplier->contact_person ?? 'Main Provider' }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <a href="{{ route('categories.index') }}"
                                class="text-[10px] font-black uppercase text-blue-600">Manage &rarr;</a>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            @foreach($categories as $category)
                            <span
                                class="px-5 py-2.5 bg-gray-50 border border-gray-100 rounded-2xl text-[10px] font-black text-gray-500 uppercase search-item search-text">
                                {{ $category->name }}
                            </span>
                            @endforeach
                        </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="p-4 bg-gray-50 rounded-2xl flex items-center gap-3 search-item group">
                                <div
                                    class="w-8 h-8 rounded-lg bg-white text-blue-600 flex items-center justify-center text-[10px] font-black shadow-sm">
                                    {{ substr($pub->name, 0, 1) }}
                                </div>
            ...
            <form id="bookForm" action="{{ route('books.store') }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    <input type="text" name="title" id="formTitle"
                        class="w-full border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500"
                        placeholder="Book Title" required>

                    <div class="grid grid-cols-2 gap-4">
                        {{-- AUTHOR --}}
                        <div class="relative combo-box" data-type="author">
                            <input type="text" list="author-list" id="authorInput" placeholder="Author"
                                class="combo-input w-full border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500">
                            <input type="hidden" name="author_id" id="authorHidden" class="hidden-id" required>
                            <datalist id="author-list">@foreach($authors as $author) <option data-id="{{ $author->id }}"
                                    value="{{ $author->name }}"> @endforeach</datalist>
                            <button type="button"
                                class="quick-add-btn hidden absolute right-3 top-3 bg-blue-600 text-white text-[9px] font-black px-3 py-2 rounded-xl">CREATE</button>
                        </div>
                        {{-- CATEGORY --}}
                        <div class="relative combo-box" data-type="category">
                            <input type="text" list="category-list" id="categoryInput" placeholder="Category"
                                class="combo-input w-full border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500">
                            <input type="hidden" name="category_id" id="categoryHidden" class="hidden-id" required>
                            <datalist id="category-list">@foreach($categories as $category) <option
                                    data-id="{{ $category->id }}" value="{{ $category->name }}"> @endforeach</datalist>
                            <button type="button"
                                class="quick-add-btn hidden absolute right-3 top-3 bg-blue-600 text-white text-[9px] font-black px-3 py-2 rounded-xl">CREATE</button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="relative combo-box" data-type="publisher">
                            <input type="text" list="publisher-list" id="publisherInput" placeholder="Publisher"
                                class="combo-input w-full border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500">
                            <input type="hidden" name="publisher_id" id="publisherHidden" class="hidden-id" required>
                            <datalist id="publisher-list">@foreach($publishers as $pub) <option data-id="{{ $pub->id }}"
                                    value="{{ $pub->name }}"> @endforeach</datalist>
                            <button type="button"
                                class="quick-add-btn hidden absolute right-3 top-3 bg-blue-600 text-white text-[9px] font-black px-3 py-2 rounded-xl">CREATE</button>
                        </div>
                        {{-- SUPPLIER --}}
                        <div class="relative combo-box" data-type="supplier">
                            <input type="text" list="supplier-list" id="supplierInput" placeholder="Supplier"
                                class="combo-input w-full border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500">
                            <input type="hidden" name="supplier_id" id="supplierHidden" class="hidden-id" required>
                            <datalist id="supplier-list">@foreach($suppliers as $sup) <option data-id="{{ $sup->id }}"
                                    value="{{ $sup->name }}"> @endforeach</datalist>
                            <button type="button"
                                class="quick-add-btn hidden absolute right-3 top-3 bg-blue-600 text-white text-[9px] font-black px-3 py-2 rounded-xl">CREATE</button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <input type="number" name="year_published" id="formYear"
                            class="border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500"
                            placeholder="Year Published (YYYY)">
                        <input type="text" name="place_published" id="formPlace"
                            class="border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div id="copiesFieldWrapper">
                            <input type="number" name="copies_available" id="formCopies" value="1" min="1"
                                class="w-full border-none bg-gray-100/50 rounded-2xl p-5 font-bold focus:ring-2 focus:ring-blue-500"
                                required>
                        </div>
                    </div>
                ...
                <button type="submit" id="formSubmitBtn"
                    class="w-full bg-gray-900 hover:bg-black text-white font-black py-5 rounded-[2.5rem] shadow-2xl transition-all active:scale-95 uppercase tracking-widest text-sm">Register
                    Book</button>
            </form>
        </div>
    </div>
