    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->books as $bookId => $data) {
            ...
            foreach ($data['copy_ids'] as $copyId) {
                ...
                if (!$copy || in_array($copy->status, ['borrowed', 'lost', 'damaged'])) continue;

                // Create Borrow with 3-day limit
...
{
    // Load relationships to ensure we have data for the log
    ...
    // New: Activity Log for the Return Transaction
    ActivityLog::create([
        'user_id'     => Auth::id(), // The Admin processing the return
        'action'      => 'return',
